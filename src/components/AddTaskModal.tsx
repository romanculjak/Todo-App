...
import { add, Status } from '../features/todo/todoSlice';
...
function AddTaskModal({isOpen, setOpen}: Props) {

    const [titleHolder, setTitle] = useState<string>("New Task")
    ...
    const handleSubmit = (e : React.SyntheticEvent)=> {
        ...
        dispatch(add({id:uuid(), title:titleHolder, status:statusHolder}))
    ...
    const addEnabled = titleHolder.length > 3;
        ...
        <div className='absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-lg p-2'>
            <div className='bg-white p-4 w-full rounded-md'>
                ...
                <form className='flex flex-col gap-2' id='editForm' onSubmit={(e)=>handleSubmit(e)}>
                    ...
                    <label className='flex flex-col gap-2'> 
                        Status
                        <select className='bg-slate-200 p-2 text-gray-800 focus:outline-indigo-500' onChange={(e)=>statusChanged(e)}>
                ...
                <div className='mt-8'>
                    <button className='button button--primary mr-2' disabled = {false} type='submit' form='editForm'>Add task</button>
                </div>
            </div>
        </div>
    </div>
